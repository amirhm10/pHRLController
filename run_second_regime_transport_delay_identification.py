...
METHOD_NAME = "second_regime_transport_delay_identification"
TRAIN_FRACTION = 0.70
MAX_VOLUME_ML = 60.0
GRID_STEP_ML = 0.5

# Sessions 4-6 are the later operating regime with roughly 69-70 s sampling.
MIN_ONE_MINUTE_SESSION_ID = 4
...
def main() -> None:
    ...
    preprocessed = select_one_minute_second_regime(preprocessed_full)
    ...
    subset_summary = make_second_regime_subset_summary(preprocessed_full, identified)
    ...
    print(f"Second-regime transport-delay identification complete: {output_dir}")
...
def select_one_minute_second_regime(preprocessed: pd.DataFrame) -> pd.DataFrame:
    subset = preprocessed.loc[
        preprocessed["session_id"].ge(MIN_ONE_MINUTE_SESSION_ID)
    ].copy()
    if subset["trial_id"].nunique() < 2:
        raise ValueError("The second-regime subset needs at least two trials.")
    return subset


def make_second_regime_subset_summary(
    ...
    return pd.DataFrame([{
        "subset_name": "one_minute_second_regime",
        "filter_rule": f"session_id >= {MIN_ONE_MINUTE_SESSION_ID}",
...
def save_tables(
    ...
    tables = {
        ...
        "second_regime_subset_summary": (
            table_dir / "second_regime_subset_summary.csv"
        ),
    }
    ...
    subset_summary.to_csv(tables["second_regime_subset_summary"], index=False)
    return tables
